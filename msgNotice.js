...
ContentMsgNotice.prototype.buildTemplate = function (type, content) {
  return `
    <div class="ext__msgNotice ${type}" style="top: ${
    this.top ? `${this.top}px` : "auto"
  }">
...
ContentMsgNotice.prototype.removeNode = function () {
  ...
  if (len > 1) {
    for (i = index; i < len - 1; i++) {
      contentMsgNoticeInstances[i].$node[0].style.top =
        parseInt(contentMsgNoticeInstances[i].$node[0].style.top, 10) -
        removedHeight -
        14 +
        "px";
    }
